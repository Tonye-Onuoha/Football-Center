...
class CommentsModelTest(TestCase):
	...
	def setupTestData(cls):
		user = User.objects.create_user(username='john',email='johndoe@gmail.com',password='zGXn9dk143')
		message = Comments(post='My first post',author=user)
		message.save()
    ...
    def test_post_author(self):
        comment = Comments.objects.all().first()
        expected_user = 'john'
        self.assertEqual(comment.author,expected_user)
        
    def test_post_content(self):
        comment = Comments.object.get(id=1)
...
class RepliesModelTest(TestCase):
    ...
    def setupTestData(cls):
        ...
        post.save()
        Replies.objects.create(reply='I replied your post',comment=post,user=user_2)
        ...
        def test_help_text(self):    reply = Replies.objects.get(id=1)
...
class QuotesModelTest(TestCase):
    ...
    setupTestData(cls):
        ...
        def test_post_user(self):
            quote = Quotes.objects.get(id=1)
            expected_username = 'john'
            self.assertEqual(quote.post.author,expected_username)
            
        def test_quote_user(self):
            quote = Quotes.objects.get(id=1)
            expected_username = 'jane'
            self.assertEqual(quote.user,expected_username)
...
class NotificationModelTest1(TestCase):
    @classmethod
    def setupTestData(cls):
        user_1 = User.objects.create_user(username='john',email='johndoe@gmail.com',password='zGXn9dk143')
        user_2 = User.objects.create_user(username='jane',email='janedoe@gmail.com',password='sBQl6fk085')
        post = Comments.objects.create(post='I am writing a test',author=user_1)
        Replies.objects.create(reply='This is your first reply',comment=post,user=user_2)
...
class NotificationModelTest2(TestCase):
    @classmethod
    def setupTestData(cls):
        user_1 = User.objects.create_user(username='john',email='johndoe@gmail.com',password='zGXn9dk143')
        user_2 = User.objects.create_user(username='jane',email='janedoe@gmail.com',password='sBQl6fk085')
        comment = Comments.objects.create(post='I am writing a test',author=user_1)
        Quotes.objects.create(quote='This is your first quote',post=comment,user=user_2)        
        def test_notification_exists(self):
            notification = Notifications.objects.all()
            expected_size = 1
            self.assertEqual(len(notification),1)
